 ...
 *****************************************************************************
 *  @file		CAN_LPC18xx.c
 *  @brief	CAN Driver, Hardware specific module for NXP LPC18xx
 *  @date		2/01/2013
...
//changes in CAN_LPC18xx.c from original file were in below functions
#define Bus_off_mode_flag   128
#define WARNING_STATUS_FLAG  64
#define PASSIVE_ERROR_FLAG   32
...
#define C_CAN0_CLK           120000000
#define C_CAN1_CLK           120000000
...
void CAN_ResetInit(void)
{
	/* 
	Work around configuring the CAN pins as inputs and cutting down the Peripheral clock 
	I2S writes are giving spurious
	*/
  LPC_CCU1->CLK_APB3_CAN0_CFG = 0;
	while ((LPC_CCU1->CLK_APB3_CAN0_CFG&1)); /* Wait for run disable     */
	/* Configure the CAN Pins as inputs for solving the issue in CAN peripheral in the lpcware */
  LPC_SCU->SFSP3_1  =        5  | 			/* P3_1 - CAN0_RD                    */
                             (1 << 4) | /*        EPUN = 1 (disable pull-up) */
                             (1 << 6) ; /*        EZI  = 1 (input buffer en) */
	LPC_SCU->SFSP3_2  =        5  | /* P3_2 - CAN0_TD                    */
                             (1 << 4) | /*        EPUN = 1 (disable pull-up) */
														 (1 << 6) ; /*        EZI  = 1 (input buffer en) */
}
/*--------------------------- CAN_hw_setup ----------------------------------
...
void CAN_Reset_Bus_Mode(U32 ctrl)
{
        LPC_C_CANn_Type *CANx = CAN_CTRL[ctrl-1];
        CANx->CNTL  &=  0xFE;
}
...
void CAN0_IRQHandler (void) {
  ...
  U32      ch, stat, nd;
   ...
   while (1) {
    ...
    stat = CANc->STAT;
    if(stat & Bus_off_mode_flag   )
    {
    	  CAN_Reset_Bus_Mode(1);
    }
     ...
     if ((ch && ch <= 0x20) )  {
      if (rtr_mo[CTRL0] & (1 << (ch-1))) {     /* If RTR transmitted or 
        ...
        CANc->STAT = stat & ~(1 << 3);         /* Clear TX status            */
				//isr_evt_set(CAN_TX_KEY, CAN_TX_ID); 
      }
        ...
        CANc->STAT = stat & ~(1 << 4);         /* Clear RX status            */
		//	isr_evt_set(CAN_RX_KEY, CAN_RX_ID); 
      }
     } else if (ch) {                           /* If error interrupt         */
       CANc->STAT = stat & ~(7 << 0);           /* Clear LEC status           */
    }
		
		 if(stat & WARNING_STATUS_FLAG)
    {
    	//  myPrintf("WARNING STATUS");
			  stat = stat & ~(0x40 << 0);
        CANc->STAT = 	stat;		
			 // CAN_REinit();
			//CAN_Reset_Bus_Mode(1);
    }
    if(stat & PASSIVE_ERROR_FLAG)
    {
    	//  myPrintf("PASSIVE ERROR");
			 stat = stat & ~(0x20 << 0);
        CANc->STAT = 	stat;	  
			 // CAN_REinit();
			//CAN_Reset_Bus_Mode(1);
